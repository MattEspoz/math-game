...
// Check Local Stoage for Best Scores, set best scores
...
function scoresToDOM() {
  ...
  baseTimeEl.textContent = `Base Content: ${baseTime}`;
  penaltyTimeEl.textContent = `Penalty: +${penaltyTime}s`;
  finalTimeEl.textContent = `${finalTimeDisplay}s`;
  updateBestScore();
  // Scroll to Top of Equations
  itemContainer.scrollTo({ top: 0, behaviour: 'instant'})
  showScorePage();
}

// Stop Timer, Go to Score Page
function checkTime() {
...
// Add a 1/10 of a second to a the Time
function addTime() {
  timePlayed += 0.1;
  checkTime();
}

// Start Timer When Game Play is Clicked
function startTimer() {
  // Reset Times
  timePlayed = 0;
  penaltyTime = 0;
...
function createEquations() {
  ...
  console.log('wrong equatoins:', wrongEquations);
